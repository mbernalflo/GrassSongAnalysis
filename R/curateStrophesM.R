#' Function for curating automatically detected strophe cutoffs (manual verision).
#' 
...
curateStrophesM = function(envFine, wavfile, stropath, stroCutoffs=NULL, f=44100, wl=44, ovlp=50, padding=TRUE, 
  ...
  if(manualEdit) {
    ...
    print("Locate new thresholds in plot (x coordinates will be shown on exit) ...")  
    newCutoffs <- locator()$x
    if(is.null(newCutoffs)) stop(paste0("No changes to file ", wavfile, "."))
    print(newCutoffs)
    stroCutoffs[,4:8] = NA
